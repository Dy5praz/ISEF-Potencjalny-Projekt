# 08 — Konkursy: Explory, ISEF, rozbiegowe

**Stan na 21 sierpnia 2026.** Wszystkie liczby odczytane w dokumentach regulaminowych, nie w streszczeniach. Zastępuje `archiwum/08_KONKURENCJA_ISEF.md`, `archiwum/31_ANALIZA_STAWKI_2026.md`, `archiwum/ISEF_ARKUSZE_OCENY.md` i ściągawkę z handbooka. Szczegóły konkursów rozbiegowych: `archiwum/33_KONKURSY_ROZBIEGOWE.md`.

---

## 1. Ścieżka — jeden strzał, i to ten wcześniejszy

`[fakt, Regulamin Konkursu Explory §8 pkt 7c]`

> „Reprezentowanie Polski na Regeneron ISEF oznacza przyznanie przez Organizatora nagrody w postaci tygodniowego wyjazdu na Regeneron ISEF, **który odbędzie się w maju kolejnego roku** w USA"

`[fakt, §8 pkt 5]` Organizator ma **wyłączność krajową**. **Explory jest jedyną drogą na ISEF z Polski.**

> **Finał Explory X 2027 → ISEF maj 2028.**

**Dlaczego liczy się tylko ISEF 2028:** aplikacje na studia w USA składa się jesienią 2028, decyzje zapadają w marcu 2029. **ISEF 2029 jest po decyzjach.** Rok drugi zachowuje wartość konkursową i naukową, ale **nie rekrutacyjną**.

---

## 2. Explory — kalendarz i kryteria

### 2.1 Terminy (§6)

| Etap | Termin |
|---|---|
| rekrutacja | **1 IX – 28 II** |
| wyniki kwalifikacji | do 31 III |
| **półfinał** | **5 V – 30 VI**, trwa 4–5 dni, **w całości online** |
| lista finalistów | do 30 VI |
| **finał** | **1–31 X**, Gdynia |

`[fakt, §11 pkt 2]` *„Terminy podane w Regulaminie to **terminy instrukcyjne** Organizatora"* — w edycji 2026 rekrutację przedłużono do 15 marca. **Planować pod 28 II, nie panikować przy poślizgu.**

`[fakt, §3]` **Data Kwalifikacji** przesuwa się o rok w każdej edycji. Dla edycji 2027: urodzeni w/po **1 I 2006**.

### 2.2 Kryteria oceny (§7) — i gdzie naprawdę są punkty

**Kwalifikacja**, 3 × 0–10 pkt: znajomość tematu · dodatkowa wartość naukowa/społeczna/gospodarcza · praktyczne zastosowanie.

**Półfinał**, 4 × 0–10 pkt:
- a) projekt jest innowacyjny, nowatorski **i/lub wnosi dodatkową wartość** — **10 pkt**
- b) ciekawa i klarowna prezentacja — **10 pkt**
- c) znajomość zastosowanych metod i założeń — **10 pkt**
- d) **„zna dotychczasowe badania dotyczące danej dziedziny" — 10 pkt**

`[wniosek]` **Punkt d waży tyle samo co innowacyjność.** Cały materiał do niego jest gotowy w `05_STAN_WIEDZY.md`. To jest najlepiej opłacona praca w całym projekcie.

**Finał**, 3 obszary × 0–10 pkt: **doskonałość merytoryczna i jakość wykonania** · **praktyczna stosowalność** · **społeczne oddziaływanie**.

`[fakt]` Kryterium „praktyczna stosowalność" pkt a brzmi *„proponuje nowatorskie rozwiązanie **lub twórczo rozwija już istniejące**"* — **przyznanie się do obsadzonego pola nie kosztuje punktów.**

### 2.3 Nagrody (§8)

| Nagroda | Kwota |
|---|---|
| Nagroda Główna | **15 000 zł** |
| Nagrody Zrównoważonego Rozwoju | **trzy kwoty do 5 000 zł** |
| Nagrody Specjalne Partnerów | 3 000 zł |

`[fakt]` Nagrody SDG przysługują trzem obszarom: **Człowiek i Społeczeństwo, Klimat i Środowisko, Gospodarka i Bezpieczeństwo**. **Obszar „Poza kategoriami" nagrody SDG nie ma.**

**Obszar wybrany: Człowiek i Społeczeństwo**, cel SDG 3 albo 10, historia o dostępności.

### 2.4 Zakazy (§4 pkt 5)

Projekty wykorzystujące szkodliwe substancje, zakładające **inwazyjne** badania na zwierzętach lub ludziach, dotyczące tematyki militarnej.

`[fakt]` **Zakaz dotyczy badań inwazyjnych, nie wszystkich badań z udziałem ludzi. Nieinwazyjne EEG jest dozwolone.**

---

## 3. Lejek i realne szanse

`[fakt, informacja prasowa FZT z 1 VI 2026]` Edycja 2026: **377 zgłoszeń → ponad 130 półfinał → 20 finał + 1 z plebiscytu → 3 na ISEF.**

`[fakt, §6 pkt 11]` **Druga droga do finału: Plebiscyt Internetowy „Bilet na Finał".** Zwycięzca wchodzi do finału. W 2026 wygrał projekt z **904 głosami** przy ~18 650 oddanych. **To jest realna, druga ścieżka przez najwęższe gardło i zależy od zupełnie innych czynników niż ocena jury.**

`[domysł]`, błąd rzędu ×1,5 w każdą stronę. **Przeliczone od nowa 21 VIII 2026 — poprzednia wersja tabeli była niezgodna z własnymi wskaźnikami bazowymi o więcej, niż wynosi ten margines. K-107.**

**Wskaźniki bazowe z edycji 2026, do których każdy szacunek musi się dać sprowadzić:**

| Przejście | Baza 2026 |
|---|---|
| zgłoszenie → półfinał | 130 / 377 = **34,5%** |
| półfinał → finał | 21 / 130 = **16,2%** |
| finał → ISEF | 3 / 21 = **14,3%** |
| finał → Nagroda Główna | 1 / 21 = **4,8%** |

**Szacunek dla tego projektu — z jawnym łańcuchem, żeby dało się go sprawdzić.**

> **UWAGA, K-110: cała poniższa tabela jest WARUNKOWA — obowiązuje, jeżeli projekt powstanie.** Warunek wchodzi pierwszym ogniwem („projekt z wideo, urządzeniem i liczbami") i dziedziczą go wszystkie następne. **Liczby bezwarunkowe: §3.0a poniżej.**

| Krok | Szansa `[domysł]` | Skąd |
|---|---|---|
| półfinał | **~85%** | projekt z wideo, urządzeniem i liczbami; baza 34,5% dotyczy całej stawki, w tym zgłoszeń szczątkowych |
| finał, **warunkowo** | ~55% | oszacowanie 24–26 pkt plasuje w grupie dziesięciu walczących (§3.1); baza 16,2% |
| **finał, łącznie z półfinałem** | **~47%, w zaokrągleniu ~50%** | 0,85 × 0,55 |
| ISEF, **warunkowo z finału** | ~28% | baza 14,3%, podniesiona dla projektu z górnej połowy stawki |
| **reprezentacja na ISEF** | **~14%** | 0,47 × 0,28 |
| **Nagroda Główna Explory** | **~5%** | 0,47 × ~10% (baza 4,8%, podniesiona) |
| **jakakolwiek nagroda na ISEF** | **~4%** | 0,14 × ~27% (odsetek nagradzanych finalistów ISEF) |
| **wartość dla aplikacji na studia, niezależna od wyniku** | **~100%, jeżeli projekt powstanie** | — |

`[fakt]` **Co było źle.** Tabela podawała **ISEF ~22%** i **Nagrodę Główną ~9–10%** przy jednoczesnym **finale ~50%**. Z bazy 3/21 wynika, że przy 50% szansy na finał ISEF wychodzi **~7%** przy wskaźniku bazowym i **~14%** przy szczodrym założeniu, że projekt jest wyraźnie powyżej średniej finalisty. **Do 22% brakowało jeszcze mnożnika 1,6 — czyli tabela przekraczała własny zadeklarowany margines ×1,5.** Ta sama nieścisłość dotyczyła Nagrody Głównej (9–10% wobec wyprowadzalnych ~5%).

`[fakt]` **I była wewnętrznie sprzeczna między plikami:** `07_HARMONOGRAM.md` w sekcji „Cel nadrzędny" niósł od 16 VIII **~14%** — liczbę poprawną — podczas gdy `08`, `README.md` i `10_STUDIA_USA.md` niosły 22%. **Cztery pliki, dwie liczby, żadnego wpisu o rozbieżności.**

### 3.0a Cała powyższa tabela jest WARUNKOWA — K-110, 21 VIII 2026

`[fakt]` Pierwsze ogniwo łańcucha, *„półfinał ~85%"*, jest uzasadnione zdaniem **„projekt z wideo, urządzeniem i liczbami"**. **To jest warunek, i dziedziczą go wszystkie ogniwa poniżej.**

`[domysł]` Wyprowadzenie: **`13_WERDYKT.md` §1.2**. **P(projekt powstaje w wersji z wynikiem, ranga C lub wyżej) = ~55%** — iloczyn trzech bramek: E0 **0,89** · kalendarz **0,80** · nieporzucenie **0,80**.

| Cel | Warunkowo | **Bezwarunkowo** |
|---|---|---|
| finał Explory | ~50% | **~33%** |
| **reprezentacja na ISEF** | ~14% | **~9%** |
| Nagroda Główna Explory | ~5% | **~3%** |
| jakakolwiek nagroda ISEF | ~4% | **~2%** |

**Kontrola:** 0,33 × 0,28 = **9,4%**, zgodne z 8,9% liczonym wprost. **Baza dla zgłoszenia losowego: 3/377 = 0,8%** — projekt siedzi jedenaście razy powyżej niej.

`[luka]` **Najsłabsze ogniwo to nie żadna z tych liczb, tylko „finał warunkowo ~55%"** — stoi na samoocenie 24–26 pkt wobec grupy walczącej 21–25 (§3.1), której nikt z zewnątrz nie sprawdził. `[domysł]` Przy zawyżeniu tego mnożnika o połowę **ISEF spada do 4–5%. Uczciwy przedział: 4–9%, a 14% jest sufitem.**

### 3.1 Z kim się konkuruje — stawka finałowa 2026

`[fakt]` Noty 21 plakatów finałowych w skali finałowej (3 × 0–10). **Grupa realnie walcząca: dziesięć projektów w przedziale 21–25 punktów.** Górny koniec to 25 (kwantowe wspomaganie fotowoltaiki, BIO-VOLT).

**Oszacowanie dla tego projektu: 24–26 punktów** przy dowiezieniu planu i treningu prezentacyjnym. **Zrównanie się z czołówką realne, wyraźne pobicie — nie.**

**Cztery obserwacje ze stawki, warte skopiowania:**

1. **Rygor pomiarowy sam w sobie nie wprowadza do finału.** Nanokompozyt z SPION (SEM, EDS, XRD, dwie mentorki) — lista rezerwowa. Kolano pneumatyczne (jeden render CAD, ankieta szkolna) — finalista. `[domysł, n=1]` Hipoteza: projekt oparty na aparaturze, której licealista nie posiada, rodzi pytanie **„ile z tego zrobiłeś ty"**
2. **O półfinale nie decyduje plakat, tylko wideo i prezentacja online.** Finalista z plakatem w czasie przyszłym i zerem wyników; lista rezerwowa z niepodmienionym tytułem szablonu
3. **Tabela z liczbami z wielu warunków odróżnia górę od dołu.** Z dziesięciu najlepszych **siedem** ma własną tabelę albo wykres; z dziesięciu najniższych — **zero**
4. **Raportowanie wyników negatywnych obok pozytywnych** robi tylko jeden projekt w całej stawce. To jest przewaga wiarygodnościowa do wzięcia za darmo

### 3.1a Narracja na finał — strategia autora, rozebrana 21 VIII 2026 (K-114)

**Diagnoza autora:** w jury Explory nie ma nikogo od interfejsów mózg–komputer; są inżynierowie, w tym od elektroniki, plus **część laicka**. Wniosek: przekonać ich historią osób, którym urządzenie mogłoby przywrócić namiastkę samodzielności.

`[wniosek]` **Diagnoza trafna i poparta danymi ze stawki 2026** (obserwacja 1 poniżej): nanokompozyt z pełnym warsztatem analitycznym poszedł na listę rezerwową, kolano pneumatyczne z jednym renderem CAD zostało finalistą. **Dwie z trzech rubryk finałowych są nietechniczne, a cały rozrzut noty tego projektu (23 → 26) siedzi właśnie w nich** (`13_WERDYKT.md` §0.1).

**Trzy ryzyka i sposób ich zamknięcia:**

**1. Przepaść między obietnicą a urządzeniem.** Trzy poruszające historie, a potem osiem celów przy 70% w laboratorium. `[wniosek]` **Inżynier w jury żyje dokładnie w tej szczelinie**, a projekt ma zasób, którego nikt inny w stawce nie ma — uczciwość (wyniki negatywne, przyznanie się do zabicia własnej osi). **Nadobietnica wydaje ten zasób w całości.**

**2. Zarzut o kamerkę wraca ostrzejszy — i tu jest jedyne kryterium doboru, jakie się liczy.**

> `[wniosek]` **Wszystkie trzy osoby muszą być takie, u których śledzenie wzroku kamerką nie działa.** Nie dowolne osoby sparaliżowane — takie, które **straciły albo tracą kontrolę nad ruchami gałek ocznych** (późne stadium SLA z oftalmoplegią, zespół zamknięcia z niepewnym spojrzeniem).
>
> **Wtedy zarzut o kamerkę umiera wewnątrz samej historii, zanim ktokolwiek zdąży go postawić.** Historia o kimś, kto steruje wzrokiem sprawnie, **sama podkłada jurorowi najgroźniejszy zarzut projektu.**

**3. R13 i pokaz.** Po trzech emocjonalnych historiach **nieudany pokaz na ochotniku spada z dużo większej wysokości** (39% u Fodora). Zakaz demonstracji na jurorze jako jedynej formy pokazu robi się przez tę strategię **ważniejszy, nie mniej ważny.**

**Struktura, która zamyka wszystkie trzy naraz — historia uzasadnia OGRANICZENIE, nie WYNIK:**

| Wersja słabsza | **Wersja mocna** |
|---|---|
| „Tym ludziom to urządzenie przywróci samodzielność" | „Ta osoba nie założy czepka z przewodami. **Dlatego urządzenie musi być małe.** Małe znaczy, że elektroda odniesienia nie ma dokąd pójść. **To zmierzyłem, oto liczba.**" |
| historia stoi **obok** projektu | historia **wchodzi** w projekt i kończy się na tabeli wyników |
| obiecuje skutek niedowieziony | **opisuje ograniczenie faktycznie zmierzone** |

`[wniosek]` **W tej wersji nic nie jest obiecane na wyrost, a laik i tak dostaje całą emocję — bo ograniczenie jest ludzkie, nie techniczne.** I rubryka merytoryczna zaczyna pracować na dwie miękkie, zamiast leżeć obok nich.

**Źródło historii: relacje już opublikowane, nie własne rozmowy** — `09_FORMALNOSCI.md` §1.2. Zwolnione z IRB w całości, sprawdzalne przez jurora, cytowalne.

**Prezentacja przed publicznością, równolegle do finału:** `[wniosek]` osobna okazja i **osobny materiał** — audytorium w całości laickie, więc udział historii może być większy. **Wersja dla jury zostaje ta z tabelą na końcu.**

---

**Sześć rzeczy do przeniesienia wprost do materiałów:** tabela wieloczynnikowa na plakacie · liczba w konkluzji zamiast przymiotnika · wyraz **„grupa kontrolna"** wypisany wprost · raportowanie wyników negatywnych · historia osobista albo lokalna w motywacji · **nie pokazywać aparatury, której nie umie się obsłużyć samodzielnie**.

---

## 4. ISEF — arkusz oceny

`[fakt, `societyforscience.org/isef/grand-award/criteria/`]` **Arkusz inżynierski**, właściwy dla tego projektu:

| Sekcja | Punkty |
|---|---|
| I. Research Problem | **10** |
| II. Design and Methodology | **15** |
| III. Execution: Construction and Testing | **20** |
| IV. Creativity & Potential Impact | **20** |
| V. Presentation — **a. Poster 10 · b. Interview 25** | **35** |

`[wniosek]` **Najważniejsza liczba w tym pliku: plakat wart jest 10 punktów ze 100, a rozmowa z jurorem 25 — więcej niż jakakolwiek inna pojedyncza pozycja.**

### 4.0a Dwie różne ligi, których nie wolno mylić — dopisane 21 VIII 2026, K-113

**Uwaga autora:** *„rozróżniaj miejsca w kategorii i ogólnie, bo to trochę mylące."* Trafna — poprzednie wersje tego pliku i `13_WERDYKT.md` używały słowa „nagroda" dla obu rzeczy naraz.

`[fakt, societyforscience.org]`

| Liga | Co to jest |
|---|---|
| **Grand Awards — W KATEGORII** | każda z **22 kategorii** ma własne miejsca **I–IV**: **$6 000 / $2 400 / $1 200 / $600**. **Liczba nagród skaluje się z liczbą projektów w kategorii** |
| **Top Awards — OGÓLNIE** | Gordon E. Moore Award **$75 000**, Regeneron Young Scientist **$50 000**, **Best of Category $5 000** za każdą kategorię |

`[fakt]` Finalistów jest **ponad 1 600** w 22 kategoriach, czyli rzędu **70–80 projektów na kategorię**.

**Kalibracja, którą ten projekt ma u siebie i nie używał jej dotąd do niczego:** `[fakt]` **ENBM074 dostał Second Award — $2 400, czyli drugie miejsce W KATEGORII — mając sprzęt kupiony za 1 800 USD i nie zbudowawszy niczego.** Wygrał **wyłącznie starannym planem eksperymentalnym**: randomizacja, kontrbalansowanie, replikacja, poprawka na wielokrotne porównania, test mechanizmu przez podwójną dysocjację.

`[wniosek]` **To jest najmocniejszy punkt odniesienia, jaki projekt posiada.** Ten projekt ma **to samo rzemiosło** (skopiowane świadomie), **plus własny tor analogowy**, **plus własny pomiar**, **plus przewidywanie ilościowe zapisane z góry.** **Drugie miejsce w kategorii EBED jest w zasięgu — nie „nie należy pod to planować".**

`[luka]` **Czego nie wiem i nikt tego nie publikuje: przełożenia noty z arkusza na miejsce.** Rozkład punktów w kategorii nie jest udostępniany. **Każde oszacowanie miejsca — moje i autora — stoi na tym samym braku danych i żadne nie jest mocniejsze od drugiego.**

**Czego arkusz NIE ma: rubryki nowości.** Ani inżynierski, ani naukowy.

**Podpunkty, które ten projekt trafia wprost:**
- `Design and Methodology`: *„**exploration of alternatives**"* — `11_EWOLUCJA.md` jest gotowym materiałem
- `Execution`: *„prototype has been **tested in multiple conditions/trials**"* — plan pomiarowy w czystej postaci
- `Creativity`: *„**Judges should place emphasis on research outcomes and analysis** in evaluating creativity"* — pomiar bije efektowny pomysł
- `Interview`: *„understanding **interpretation and limitations** of results"* oraz *„**degree of independence** in conducting project"*

### 4.1 Kategoria: EBED

`[fakt]` Definicja: *„Studies involving **electrical systems in which information is conveyed via signals and waveforms** for purposes of enhancing **communications, control and/or sensing**"*. Podkategorie: Circuits, **Sensors**, **Signal Processing**, Microcontrollers.

**Trzy skutki operacyjne:**
1. **Sędzia jest elektronikiem, nie lekarzem.** Materiały prowadzą przez tor sygnałowy, szum, pasmo, przetwornik i dekodowanie — fizjologia jest tłem
2. **Nie opisywać urządzenia jako medycznego.** Ani diagnostycznego, ani terapeutycznego, ani „dla pacjentów"
3. **Procedura Human Participants obowiązuje mimo wszystko** — kategoria nie zmienia tego, że sygnał zbiera się z człowieka

`[fakt]` Dla porównania ENBM to *„projects that aim to improve human health (…) for **clinical and public health use**"* — projekt, który świadomie nie jest sprzedawany jako medyczny, broniłby się tam gorzej.

### 4.2 Reguła dwunastu miesięcy

`[fakt, rocznik 2026–2027]` *„no more than 12 months of continuous research (…) beginning no earlier than January [roku N−1] and ending May [roku N]"*. **Osiemnastu miesięcy nie ma nigdzie.**

`[wniosek]` Dla ISEF maj 2028 okno wyniesie **I 2027 – V 2028**. **Formalna kampania pomiarowa startuje w maju 2027**; wszystko wcześniejsze to prace rozwojowe. El-Robo-Mech i półfinał Explory reguły 12 miesięcy nie mają.

`[luka]` Regulamin rocznika 2027–2028 jeszcze nie istnieje — ukaże się ~połowa 2027. **Przeczytać od nowa po publikacji.**

### 4.3 Konkurencja na ISEF — sprawdzona u źródła

Baza abstraktów, **trzynaście roczników**: `SSVEP` — **5 trafień**, `ADS1299` — **0**. Wszystkie pięć to zastosowania. **Nikt w trzynastu latach nie zbudował na ISEF własnego wzmacniacza EEG.** Rozbiór: `05_STAN_WIEDZY.md` §8.

### 4.4 Projekt referencyjny — ENBM074 (2026)

`[fakt, baza abstraktów]` Kharade, Ameya, Nashua High School South — *„Breaking the Brain-Computer Interface Ceiling"*, **Second Award $2 400**.

Sprzęt **kupiony za 1 800 USD**, nic nie zbudowane. Twierdzenie o **paradygmacie**: rozstrzyganie intencji zamiast literowania. 65 wpm wobec 3 wpm, 111 prób randomizowanych i kontrbalansowanych **na tym samym sprzęcie**, replikacja na drugiej grupie, poprawki na wielokrotne porównania, Cohen's d, **test mechanizmu przez podwójną dysocjację**.

**Czego się z tego uczymy — i to jest dozwolone:** ten projekt zdobył drugie miejsce **bez zbudowania czegokolwiek**, przez staranny plan eksperymentalny. **Rzemiosło kopiujemy świadomie; rozwiązania nie.**

**Ścieżka, w którą nie wchodzimy:** rozstrzyganie intencji z małego zbioru zamiast literowania, mierzone w **słowach na minutę**. Granicy pilnuje to, że produktem jest **sprzęt i pomiar**, a metryką **bity**.

---

## 5. Konkursy rozbiegowe

**El-Robo-Mech** — dwa starty: **IV 2027** (pierwszy dry-run prezentacji) i **IV 2028** (próba generalna przed ISEF). `[luka]` Regulamin edycji XII ukaże się jesienią 2026 — sprawdzić daty i czy można startować dwa razy.

**OITwEiM** — **przesunięta decyzją autora na edycję 2027/28.** `[fakt]` Bez Komitetu Szkolnego olimpiada odpada — **zapytać w szkole do 31 X 2027.**

---

## 6. Trening prezentacyjny — najwyższy stosunek punktów do godzin

`[fakt]` Waga: półfinał Explory **10 z 40** za prezentację, finał — płynność wypowiedzi w rubryce doskonałości, **ISEF Interview 25 ze 100**.

`[wniosek]` **Działa na trzy sita naraz i działa nawet przy przeciętnych pomiarach.**

| Kiedy | Runda |
|---|---|
| **IX 2027**, po kampanii pomiarowej | **diagnostyczna.** Jeżeli wyjdzie, że autor nie umie wytłumaczyć własnego warunku kontrolnego, to jest problem projektu, nie prezentacji — i zostaje miesiąc na naprawę |
| **IX–X 2027** | **adwersaryjna, intensywna.** Bank pytań, trzy poziomy: laik / inżynier spoza dziedziny / specjalista. Punktacja każdej odpowiedzi |
| **III–V 2028** | to samo **po angielsku**, plus obrona wyboru kategorii i pytanie o odróżnienie od ENBM074 |

**Bank pytań startowy — trzy gotowe odpowiedzi są już w `02_TWIERDZENIE.md` §7.**

**Ostrzeżenie:** wyuczona odpowiedź brzmi jak wyuczona i laickie jury to słyszy. **Gładki pitch rozsypujący się przy drugim pytaniu pogłębiającym jest gorszy niż odpowiedź nierówna, ale własna.**
